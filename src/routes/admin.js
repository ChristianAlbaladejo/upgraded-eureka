...
router.post('/admin/updateProducts/', md_auth.ensureAuth, (req, res) => {
    ...
    for (let i = 0; i < p.length; i++) {
        mysqlConnection.query("UPDATE product set description ='" + p[i].description + "', costPrice = '" + p[i].costPrice + "', image = '" + p[i].image + "', familyId = '" + p[i].familyId + "' WHERE id = '" + p[i].id + "'", function (error, results, fields) {
